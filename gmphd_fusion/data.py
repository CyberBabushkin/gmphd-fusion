...
class Matrix(np.ndarray):
    ...
    Source: https://stonesoup.readthedocs.io/en/v0.1b12/_modules/stonesoup/types/array.html
    ...
    def __array_wrap__(self, array, **kwargs):
...
class StateVectors(Matrix):
    ...
    def __new__(cls, states, *args, **kwargs):
        if isinstance(states, Sequence) and not isinstance(states, np.ndarray):
            if len(states) > 0 and isinstance(states[0], StateVector):
    ...
    def __iter__(self):
        statev_gen = super(StateVectors, self).__iter__()
    ...
    def _cast(cls, val):
        ...
        elif type(out) == Matrix and out.ndim == 1:
...
class CovarianceMatrix(Matrix):
    ...
    Also, it stabilizes the matrix so that the matrix is symmetrical.
    """
    ...
    @staticmethod
    def _cov_stabilization(cov: np.ndarray) -> np.ndarray:
